...
if __name__ == '__main__':
    ...
    likelihood = gpytorch.likelihoods.GaussianLikelihood(noise_constraint=gpytorch.constraints.GreaterThan(1e-6))  # 0.0001, 4e-6
    ...
    gp.train()
    likelihood.train()
    optimizer = torch.optim.Adam([{'params': gp.parameters()},
                                  {'params': recon.parameters()}], lr=0.1)
    ...
    with gpytorch.settings.max_preconditioner_size(200), gpytorch.settings.max_cg_iterations(10000), gpytorch.settings.fast_computations(log_prob=False, solves=True):
        for tx in range(5000):
            ...
            pol_hat = recon(gp.embed_input(train_xs))
            #print(pol_hat[0:2, :], train_xs[0:2,-3:])
            recon_loss = recon_loss_fn(pol_hat, train_xs[:,-3:])
            mll_loss = -mll(output, train_y) 
            loss = mll_loss + 2*recon_loss
            loss.backward()
            optimizer.step()
            #print(mll_loss.item(), recon_loss.item())
            if tx % 50 == 0:
