...
class MechanismMotion(PlotFunc):
    ...
    @staticmethod
    def description():
        return 'Plot motion generated by a mechanism'

    def _plot(self, plot_data):
        data_hist = [[],[],[],[]]
        for data_point in plot_data:
            if data_point.mechanism_params.type == 'Door':
                if data_point.policy_params.type == 'Revolute':
                    data_hist[0] += [data_point.motion]
                if data_point.policy_params.type == 'Prismatic':
                    data_hist[1] += [data_point.motion]
            if data_point.mechanism_params.type == 'Slider':
                if data_point.policy_params.type == 'Revolute':
                    data_hist[2] += [data_point.motion]
                if data_point.policy_params.type == 'Prismatic':
                    data_hist[3] += [data_point.motion]
        plt.hist(data_hist, 20, histtype='bar', label=['Door, Revolute', \
                                                        'Door, Prismatic', \
                                                        'Slider, Revolute', \
                                                        'Slider, Prismatic'])
        plt.xlabel('Motion')
